...
import { inventionPanel } from './InventionPanel';
import { applyWeaponMods, enhanceWeapon, embedRelic, isRelicId } from './WeaponMods';
import { workshopPanel } from './WorkshopPanel';
import './workshop.css';
import { CHARMS, CLASS_SECONDS, KNOWLEDGE_REQUIRED, SAVE_KEY, STUDIO_NAMES, WEAPONS, buyCharm, buyUpgrade, classCost, completeClass, completeStudio, maxStamina, newStudent, rest, sanitizeSave, unlockedStudio, upgradeCost, xpNeeded, type Charm, type Place, type StudioSave, type Upgrade, type Weapon } from './StudioState';
...
type Modal = 'invention' | 'map' | 'instructor' | 'shop' | 'clerk' | 'mystery' | 'skills' | 'tools' | 'home' | 'pause' | 'passed' | 'faculty' | 'exhibition' | 'exhibition-won' | 'character' | null;
...
export class StudioGame {
  ...
  constructor(container: HTMLElement) {
    ...
    for (const [place,file] of [['foyer','foyer-interior.png'],['home','apartment-interior.png'],['skills','student-lab.png?v=gym-20260913'],['tools','model-workshop.png?v=fabrication-20260913']] as const) {
  ...
  private equip(weapon: Weapon): void {
    ...
    this.state.weaponEquipped = this.state.weapon!==weapon || !this.state.weaponEquipped; this.state.weapon = weapon; this.persist(); this.refresh();
  }
  private changePlace(place: Place, save = true): void {
    ...
    this.gymTraining=0; this.state.place = place; this.hero.x = place === 'studio' ? 12.3 : 14; this.hero.y = FLOOR; this.hero.hp = this.state.stamina;
  ...
  private travel(place: Place, studio?: number, after: Modal = null): void {
    ...
    const title = place === 'studio' ? `L${studio ?? this.state.studio} · Studio ${studio ?? this.state.studio} · ${STUDIO_NAMES[(studio ?? this.state.studio) - 1]}` : place === 'foyer' ? 'G · Building 100 Lobby' : place === 'mystery' ? 'The Other Cupboard' : place === 'home' ? 'Your apartment' : place === 'skills' ? 'B1 · Student Gym' : place === 'tools' ? 'B2 · Model Workshop' : place === 'lobby' ? 'Back to the forecourt' : 'Everyday supplies';
  ...
  private near(): 'chair' | 'instructor' | 'exit' | 'shop' | 'mystery' | 'clerk' | 'shopfloor' | 'building' | 'lobby' | 'home' | 'skills' | 'tools' | 'map' | 'workshopfloor' | 'faculty' {
  ...
  private action(action: string): void {
    ...
    if(action==='browse-inventions'){if(this.state.place==='mystery'&&this.near()==='clerk'&&this.modal==='clerk'){this.inventionDraft=null;this.inventionWords='';this.open('invention');}return;}
    if (action === 'invent-preview' && this.state.place === 'mystery' && this.modal === 'invention' && this.near() === 'clerk') {
      const words = (this.el('invention-words') as HTMLInputElement).value;this.inventionWords=words.slice(0,120);
    ...
    if (action === 'invent-craft' && this.state.place === 'mystery' && this.modal === 'invention' && this.near() === 'clerk' && this.inventionDraft) {
      const item = this.inventionDraft;
      if (this.state.coins < CRAFT_COST || this.state.inventions[item.weapon]?.finish === item.finish) return;
      this.state.coins -= CRAFT_COST; this.state.inventions[item.weapon] = {...item}; this.state.weapon = item.weapon; this.state.weaponEquipped = true;
      this.persist(); this.open('invention'); this.refresh(); this.toast(`${item.name} crafted and equipped.`); return;
    }
  ...
  private open(modal: Modal): void {
    ...
    if (modal === 'character') {
    ...
    } else if (modal === 'invention') {
      eyebrow='MIKA · THE OTHER CUPBOARD';title='Invent with words';
      body=inventionPanel(s,this.inventionDraft,this.icons,this.inventionWords);
      ...
      body = `${gym ? `<p>Your stamina: <strong>${s.stamina} / ${maxStamina(s)}</strong>. Training uses stamina. Rest at home to recover.</p>` : `<p>You have <strong>${s.coins} coins</strong>. Earn coins from classes and completing studios.</p>`}${kinds.map(kind => {
    ...
    if(modal==='pause') {
      ...
      body=settingsMenu({tab:this.settingsTab,musicEnabled:this.music.enabled,musicVolume:this.music.volume,lowGraphics:this.lowGraphics,canVisit:!this.fight&&this.classElapsed===null&&this.sleepElapsed===null,fighting:this.fight,saveFailed:this.saveFailed,bindings:Object.entries(MENU_BINDINGS).map(([key,label])=>({action:key,label:label??key,keys:this.input.getBindings()[key as ActionName].map(prettyKey).join(' / ')}))});
    }

    if(modal==='tools') body = workshopPanel(s,this.icons);
    if(modal==='clerk') body += '<div class="clerk-services"><button data-action="browse-supplies">Everyday supplies · sneakers & lunch →</button><button class="mystery-buy" data-action="browse-inventions">Invent with words →</button></div>';
  ...
  private updateClass(dt:number):boolean {
    ...
    if(this.classElapsed>=CLASS_SECONDS){
      ...
      else if(result.ok)this.toast('+1 Knowledge · +15 XP · +10 coins. '+this.state.knowledge[this.state.studio-1]+'/'+KNOWLEDGE_REQUIRED+' learned.');
    }
  ...
  private beginAttack(): void {
    ...
    this.pendingAttack = { unarmed, weapon, damage, facing: this.facing, remaining: weapon === 'ruler' ? .15 : weapon === 'cup' ? .09 : .05, finisher };
    this.swing = .28;
  }
  private resolveAttack(): void {
    ...
    if (strike.weapon === 'cup') this.projectiles.fire(this.hero.x + strike.facing * .7, this.hero.y + 1.25, strike.facing, 17, w.range, strike.damage, false);
      ...
      if(!strike.unarmed)this.effects.slash(this.hero.x, this.hero.y, strike.facing, strike.weapon === 'ruler');
  ...
  private updateFight(dt: number): void {
    ...
    if(stepMotion(this.boss,intent.axis,difficulty.moveSpeed,dt,30,CLASSROOM_PLATFORMS,this.bossDrop>0))this.effects.dust(this.boss.x,this.boss.y);
  ...
  private positionLabels(): void {
    ...
    const visibility: Record<string, boolean> = {
      ...
      's-context-hint': !this.modal && !this.transition.active && (this.fight || this.classroomPropAvailable() ? !!this.arena.held('hero') || !!this.arena.nearby(this.hero) : this.classElapsed===null && this.sleepElapsed===null && !['lobby','shopfloor','workshopfloor','skills','tools','exit','clerk','faculty'].includes(this.near())),
    };
    this.camera.updateMatrixWorld();
    for (const [id, x, y] of [['s-archive-board',29.38,9.7],['s-crafting-label',22,FLOOR+4.8],['s-chair-label', 10, FLOOR + 2.4], ['s-boss-label', this.boss.x, this.boss.y - .22], ['s-mystery-label', 8, FLOOR + 3.4], ['s-building-label', 28, FLOOR + 3.4], ['s-clerk-label', 24, FLOOR - .22], ['s-exit-label', 3.2, FLOOR + 3.1], ['s-accommodation-sign', 54, 7.6], ['s-wayfinding', 39, FLOOR + 3], ['s-context-hint',this.hero.x,this.hero.y+4.8*(place==='lobby'?LOBBY_STUDENT_SCALE:1)]] as const) {
      ...
      if((id==='s-crafting-label'||id==='s-archive-board')&&!el.hidden){const edge=el.offsetWidth/2+8;el.style.left=`clamp(${edge}px,${(p.x+1)*50}%,calc(100% - ${edge}px))`;}
    }
  }
  private refresh(): void {
    ...
    this.label('touch-interact',this.fight || this.classroomPropAvailable()?(this.arena.held('hero')?'Throw':'Grab'):this.near()==='home'&&s.place==='home'?'Rest':this.near()==='faculty'?'Archive':this.near()==='skills'?'Train':this.near()==='tools'?'Upgrade':this.near()==='chair'?'Sit':['instructor','clerk'].includes(this.near())?'Talk':'Enter');
    ...
    for (const w of ['pen', 'ruler', 'cup'] as Weapon[]) {
      const button = this.el(`weapon-${w}`) as HTMLButtonElement; button.classList.toggle('selected', s.weaponEquipped && w === s.weapon); button.setAttribute('aria-pressed', String(s.weaponEquipped && w === s.weapon)); button.disabled = this.transition.active || this.classElapsed !== null || this.sleepElapsed !== null;
    ...
    const labels: Record<ReturnType<StudioGame['near']>, string> = { faculty:'Open professor archive', workshopfloor:'Crafting table →', chair: s.cleared[i] ? 'Studio complete · head upstairs' : 'Take a seat · attend class', instructor: s.cleared[i] ? 'Talk to your instructor' : 'Hit to start a duel · or talk', exit: s.place==='studio'?'Return to G · Building 100 Lobby':'Step outside to campus', map: 'Explore Building 100', shop: 'Browse the Supply Cupboard', mystery: 'Enter the mysterious shop', clerk: 'Talk to Mika', shopfloor: 'Walk right to Mika’s counter →', building: 'Enter Building 100', lobby: this.hero.x > 30 ? 'Accommodation → · keep walking right' : 'Shop ← · Building 100 → · home further right', home: s.place === 'home' ? 'Sleep & restore stamina' : 'Enter your apartment', skills: 'Train stamina & strength', tools: 'Modify weapon' };
