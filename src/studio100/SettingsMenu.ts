...
export function settingsMenu(state: SettingsMenuState): string {
  ...
  if (state.tab === 'general') {
    ...
    panel = `<p class="settings-note" id="binding-feedback" role="status">Select a key to change it. Escape cancels.</p><div class="settings-binding-grid">${state.bindings.map(binding => `<button class="settings-binding" data-action="rebind:${escape(binding.action)}" aria-label="Change ${escape(binding.label.toLowerCase())} key"><span>${escape(binding.label)}</span><kbd>${escape(binding.keys)}</kbd></button>`).join('')}</div><p class="settings-note settings-fixed-keys"><kbd>M</kbd> Nearby lobby lift &nbsp; <kbd>1–3</kbd> Tools &nbsp; <kbd>Esc</kbd> Menu</p>`;
  }
