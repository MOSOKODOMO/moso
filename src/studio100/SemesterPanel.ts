import { dailyActivityCost, hasActivity, KNOWLEDGE_REQUIRED, PASS_KNOWLEDGE, SEMESTER_DAYS, HD_REWARD, semesterDay, STUDIO_NAMES, WORK_PAY, type StudioSave } from './StudioState';
...
export function semesterPanel(s:StudioSave):string {
  ...
  const dayCell=(day:number,catchup=false):string=>{
    const absolute=start+day-1,current=absolute===s.day,past=absolute<s.day;
    const records=studioRecords.filter(activity=>activity.day===absolute);
    return `<li class="semester-day ${current?'current':past?'past':'future'} ${catchup?'catchup-day':''}" data-semester-day="${day}" ${current?'aria-current="date"':''}><div class="calendar-day-heading"><strong>Day ${day}</strong><small>${current?'Today':past?'Past':''}</small></div><div class="calendar-day-activities">${records.length?records.map(activity=>activityMark(activity.kind)).join(''):`<span class="calendar-day-empty">${past?'No activity logged':current?'Plan your day':'—'}</span>`}</div></li>`;
  };
  ...
  return `<div class="semester-panel"><div class="semester-topline"><div><span class="semester-studio-number">STUDIO ${String(s.studio).padStart(2,'0')}</span><h3>${escape(STUDIO_NAMES[index])}</h3></div><span class="semester-day-count ${elapsed>SEMESTER_DAYS?'is-catchup':''}">${elapsed>SEMESTER_DAYS?'Catch-up':`Day ${elapsed} / ${SEMESTER_DAYS}`}${elapsed>SEMESTER_DAYS?`<small>Day ${elapsed}</small>`:''}</span></div>
    ...
    <div class="semester-calendar-heading"><strong>${SEMESTER_DAYS}-day semester</strong><span>Sleep at home to start the next day.</span></div><ol class="semester-calendar" aria-label="Semester days 1 to ${SEMESTER_DAYS}">${Array.from({length:SEMESTER_DAYS},(_,index)=>dayCell(index+1)).join('')}</ol>
    ...
    <div class="semester-legend">${(['class','homework','work'] as ActivityKind[]).map(kind=>`<div class="semester-activity-key ${hasActivity(s,kind,kind==='work'?undefined:s.studio)?'done-today':''}">${activityMark(kind)}<strong>${ACTIVITY[kind].reward}</strong><small>${kind==='class'?'Studio · once per day':kind==='homework'?'At home · once per day':'Lucky Lantern Noodles'}</small></div>`).join('')}</div>
  </div>`;
}
