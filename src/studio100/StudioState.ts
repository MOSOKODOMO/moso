...
export const KNOWLEDGE_REQUIRED = 3;
...
export interface StudioSave {
  ...
  weapon: Weapon; weaponEquipped: boolean; owned: Weapon[]; toolRanks: Record<Weapon, number>;
...
export function newStudent(): StudioSave {
  return { version: 1, level: 1, xp: 0, coins: 90, stamina: 100, day: 1, studio: 1, place: 'lobby',
    knowledge: Array(9).fill(0), cleared: Array(9).fill(null), weapon: 'pen', weaponEquipped: false, owned: ['pen', 'ruler', 'cup'],
    toolRanks: { pen: 0, ruler: 0, cup: 0 }, endurance: 0, strength: 0, efficiency: 0, shoes: false, introSeen: false,
...
export function sanitizeSave(raw: unknown): StudioSave {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    s.knowledge[i] = s.cleared[i] === 'study' ? KNOWLEDGE_REQUIRED : integer(r.knowledge?.[i], 0, KNOWLEDGE_REQUIRED, 0);
  }
  ...
  s.weaponEquipped = r.weaponEquipped === true;
  ...
  // Credit sessions already earned under the old seven-class requirement.
...
export function completeStudio(s: StudioSave, route: Route): boolean {
  const i = s.studio - 1;
  if (s.studio > unlockedStudio(s) || s.cleared[i] || (route === 'study' && s.knowledge[i] < KNOWLEDGE_REQUIRED)) return false;
  s.cleared[i] = route;
...
export function completeClass(s: StudioSave): { ok: boolean; cleared: boolean } {
  ...
  if (s.cleared[i] || s.knowledge[i] >= KNOWLEDGE_REQUIRED || s.stamina < cost || s.studio > unlockedStudio(s)) return { ok: false, cleared: false };
...
export function buyUpgrade(s: StudioSave, kind: Upgrade): boolean {
  ...
  if ((training ? s.stamina : s.coins) < cost) return false;
